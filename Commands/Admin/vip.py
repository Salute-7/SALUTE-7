...
class vip(commands.Cog):
    ...
    @commands.slash_command(name="vip", description="Данная команда не предназначена для общего использования. (⛔)")
    async def vip_command(self, ctx, user: str):
        ...
        if str(ctx.author.id) in admin_users:
            if str(user) in vip_users:
                embed = disnake.Embed(title="Ошибка при попытке использовать команду:", description=f"{base['ICON_PERMISSION']} {user} уже является VIP-пользователем.", color=chosen_color)
                await ctx.response.send_message(embed=embed, ephemeral=True)
            else:
                vip_users[str(user)] = True
                save_vip_users(vip_users)
                embed = disnake.Embed(title="Действие выполнено", description=f"{base['APPROVED']} {user} добавлен в VIP-пользователи.", color=chosen_color)
                await ctx.response.send_message(embed=embed, ephemeral=True)
