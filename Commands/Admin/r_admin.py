...
class remove_admin(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @bot.slash_command(name="remove_admin", description="Данная команда не предназначена для общего использования. (⛔)")
    async def remove_vip_command(ctx, user: str):
        ...
        if ctx.author.id == 936292219378348033:
            if str(user) in admin_users:
                del admin_users[str(user)]
                save_admin_users(admin_users)
                embed = disnake.Embed(title="", description=f"{base['APPROVED']} {user} удален из admin-листа.", color=chosen_color)
                await ctx.response.send_message(embed=embed, ephemeral=True) 
            else:
                embed = disnake.Embed(title="Ошибка при попытке использовать команду", description=f"{base['ICON_PERMISSION']} {user} не является admin-пользователем.", color=chosen_color)
                await ctx.response.send_message(embed=embed, ephemeral=True)  
        else:
