...
class set_pay(commands.Cog):
    def __init__(self, bot):  
        self.bot = bot
        print('Файл Commands/Config/set_pay.py Загружен!')

    @bot.slash_command(name="set_pay", description="Изменить ID канала c переводами (💻)")
    async def set_pay(ctx, new_channel: disnake.TextChannel):
        ...
        if config_data is not None:
            config_data['PAY_LOGS'] = str(new_channel.id)
