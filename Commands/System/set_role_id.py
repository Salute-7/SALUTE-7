...
class set_role_id(commands.Cog):
    def __init__(self, bot):  
        self.bot = bot

    @bot.slash_command(name="set_role_id", description="Изменить ID роли, которая будет выдана при подключении (Для новых пользователей)")
    async def set_role_id(self, ctx, new_role: disnake.Role = None, delete: bool = False):
        ...
        if delete:
            if 'ROLE_ID' in config_data:
                ...
                embed = disnake.Embed(
                    ...
                    description=f"{base['APPROVED']} Значени new-role успешно отключёно.",
                    color=chosen_color
                )
                ...
                embed = disnake.Embed(
                    ...
                    description=f"{base['ICON_PERMISSION']} Значение new-role не было установлено.",
        ...
        if config_data is not None:
            config_data['ROLE_ID'] = str(new_role.id)
        ...
        embed = create_embed(
            "Действие выполнено",
            f"{base['APPROVED']} ID роли успешно изменен на {new_role.mention}.",
            color=chosen_color)
        await ctx.send(embed=embed, ephemeral=True)